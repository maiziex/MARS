...
parser.add_argument('--out_dir','-o_dir', help="The folder name you can define to store the final results",default="./Results_MSA_MARS")
...
parser.add_argument('--num_threads','-nt', help=" The number of threads you can define to run MSA by muscle",default=2)
...
parser.add_argument('--Ape_ref_list','-lr', help='The reference fasta files for each Apes which you defined in the "--Ape_name_list", respectivley. Each reference file is seperately by comma (",") ', type=str,default="")
parser.add_argument('--HARP_flag', '-h_flag',default= 0)
parser.add_argument('--gnomad_flag_for_linked_SNP', '-gnomad_flag',default= 0)
parser.add_argument('--gnomad_dir','-g_dir', help="The directory to store the gnomad snp vcf bgz file")

args = parser.parse_args()
if len(sys.argv) == 1:
    Popen("python MARS_step2_bychr.py -h",shell=True).wait()
else:
...
def Extract_fasta_for_SV(merge_SV_dict,out_dir,out_dir_chr,ref_dir,chr_num):
    ref_dict = pickle.load(open(ref_dir + "ref_seq_chr" + str(chr_num) + ".p","rb"))
    count_contig_0 = 0
    _count_miss = 0
    for key,value in merge_SV_dict.items():
        if len(value) > 1:
            ...
            _end = key[1]
            sv_type = key[2]
            fw = open(out_dir_chr + str(chr_num) + "_" + str(_start) + "_" + str(_end) + "_" + sv_type +  ".fasta","w")
...
def Multiseq_alignment_by_muscle(in_dir):
    ...
    for one_file in files_all:
        ...
        use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -in " + one_file + " -out " + output_file + " -html" + " -maxiters 1 -diags -sv -distance1 kbit20_3"
        Popen(use_cmd,shell=True).wait()

        output_file = one_file.split(".fasta")[0] + ".afa"
        use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -in " + one_file + " -out " + output_file + " -maxiters 1 -diags -sv -distance1 kbit20_3"
        Popen(use_cmd,shell=True).wait()
...
def Multiseq_profile_alignment_by_muscle(in_dir,in_dir_2):
    ...
    for one_file in files_all:
        ...
        output_file = one_file.split(".fasta")[0] + ".html"
        use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -profile -in1 " + output_file + "  -in2 " + input_file_2 +  " -out " + output_file + " -html " + " -maxiters 1 -diags -sv -distance1 kbit20_3"
        Popen(use_cmd,shell=True).wait()
...
def run_one_msa_by_muscle(one_file,output_html,output_afa,xin):
    use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -in " + one_file + " -out " + output_html + " -html" + " -maxiters 1 -diags -sv -distance1 kbit20_3"
    Popen(use_cmd,shell=True).wait()
    use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -in " + one_file + " -out " + output_afa + " -maxiters 1 -diags -sv -distance1 kbit20_3"
    Popen(use_cmd,shell=True).wait()


def run_one_profile_msa_by_muscle(one_file,output_html,output_afa_1,input_afa_2,output_afa,xin):
    use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -in " + one_file + " -out " + output_afa_1 + " -maxiters 1 -diags -sv -distance1 kbit20_3"
    Popen(use_cmd,shell=True).wait()
    use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -profile -in1 " + input_afa_2 + " -in2 " + output_afa_1  + " -out " + output_html + " -html " + " -maxiters 1 -diags -sv -distance1 kbit20_3"
    Popen(use_cmd,shell=True).wait()
    use_cmd = "/oak/stanford/groups/arend/Xin/Software/muscle3.8.31_i86linux64 -profile -in1 " + input_afa_2 + " -in2 " + output_afa_1  + " -out " + output_afa  + " -maxiters 1 -diags -sv -distance1 kbit20_3"
    Popen(use_cmd,shell=True).wait()
...
def Run_MARS_step2_chr(out_dir,out_dir_all,out_dir_table,in_dir,assembly_dir,ref_dir,ref_file,chr_start,chr_end,gnomad_flag,HARP_flag,num_of_threads):
    ...
    for chr_num in range(chr_start,chr_end + 1):
        ...
        if gnomad_flag == 1:
            ...
            Print_linked_SNPs_for_SV(out_dir_table,snp_dict,gnomad_dict,chr_num)
...
if __name__ == "__main__":
    ...
    if HARP_flag == 1:
        check_file = Ape_ref_dir + "finish_ape_ref.txt"
        if os.path.exists(Ape_ref_dir):
            ...
            Popen("touch " + bam_dir + "finish_ape_ref.txt",shell=True).wait()
    
    for chr_num in range(chr_start,chr_end):
        Run_MARS_step2_chr(out_dir,out_dir_all,out_dir_table,in_dir,assembly_dir,ref_dir,ref_file,chr_num,chr_num,gnomad_flag,HARP_flag,num_of_threads)
