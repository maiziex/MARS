    ...
    for key,value in merge_SV_dict.items():
        if len(value) > 1:
            ...
            if _start == 5105385:  #5077846:  #5098515
                print(_start)
            _end = key[1]
            fw = open(out_dir_chr + str(_start) + "_" + str(_end) + ".fasta","w")
...
if __name__ == "__main__":
    ...
    """
    for chr_num in range(chr_start,chr_end + 1):
        out_dir_chr = out_dir + "/" + "MSA_del_results_chr" + str(chr_num) + "/"
        ...
        merge_SV_dict = pickle.load(open(out_dir + "merge_DEL_dict_chr" + str(chr_num) + ".p", "rb"))
