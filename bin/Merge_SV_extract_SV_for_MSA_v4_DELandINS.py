...
if __name__ == "__main__":
    ...
    Extract_DEL_from_all_samples(sample_list,in_dir,out_dir,chr_start,chr_end)
    Extract_INS_from_all_samples(sample_list,in_dir,out_dir,chr_start,chr_end)
    merge_DEL_dict = Merge_DEL(out_dir,chr_start,chr_end)
    merge_INS_dict = Merge_INS(out_dir,chr_start,chr_end)
    
    merge_SV_dict = Merge_DEL_and_INS(out_dir,chr_start,chr_end)
    """
    for chr_num in range(chr_start,chr_end + 1):
        Align_hp_fasta_to_bam(assembly_dir,out_dir,chr_num)
    """
    for chr_num in range(chr_start,chr_end + 1):
        out_dir_chr = out_dir + "/" + "MSA_SV_results_chr" + str(chr_num) + "/"
        ...
        """
        merge_SV_dict = pickle.load(open(out_dir + "merge_SV_dict_chr" + str(chr_num) + ".p", "rb"))
